 ...
 * Sourceforge Project Page: https://www.sourceforge.net/projects/phpflickr/
 * Released under GNU Lesser General Public License (https://www.gnu.org/copyleft/lgpl.html)
 * For more information about the class and upcoming tools and toys using it,
 * visit https://www.phpflickr.com/ or https://phpflickr.sourceforge.net
 *
 ...
 *     https://www.phpflickr.com/README.txt
 *
 *     Please submit all problems or questions to the Help Forum on my project page:
 *         https://sourceforge.net/forum/forum.php?forum_id=469652
 *
 */
if (session_id() == "") {
    @session_start();
...
class phpFlickr {
    ...
    function auth ($perms = "read", $remember_uri = true)
    {
        ...
        if (empty($_SESSION['phpFlickr_auth_token']) && empty($this->token)) {
			...
			if ($this->service == "23") {
				header("Location: https://www.23hq.com/services/auth/?api_key=" . $this->api_key . "&extra=" . $redirect . "&perms=" . $perms . "&api_sig=". $api_sig);
			} else {
				header("Location: https://www.flickr.com/services/auth/?api_key=" . $this->api_key . "&extra=" . $redirect . "&perms=" . $perms . "&api_sig=". $api_sig);
			}
            exit;
        } else {
